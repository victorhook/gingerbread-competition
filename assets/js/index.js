...
$(document).ready(() => {
    ...
    for (let popup of $('.popup')) {
        let btn = makeExitButton(popup);
        lookForCloseBtns(popup, popup);
        $(popup).hide();
    }
    ...
    function lookForCloseBtns(popup, currNode) {
        if (currNode.children.length == 0)
            return;
        for (let child of [ ...currNode.children ]) {
            if ([ ...child.classList ].indexOf('popup-close') > 0) {
                child.addEventListener('click', () => closePopup(popup));
            }
            lookForCloseBtns(popup, child);
        }
    }

    function makeExitButton(popup) {
        let btn = document.createElement('button');
        btn.classList.add('popup-exit-btn');
        btn.innerHTML = 'X';
    ...
    class CandyCane {
        constructor() {
            ...
            this.image.src = '/gingerbread-competition/assets/images/cane.svg';
        }
            render(ctx) {
                if (cursorDown) {
